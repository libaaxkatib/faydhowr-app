...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('store_order_status_histories', function (Blueprint $table): void {
            ...
            $table->index(['store_order_id', 'created_at']);
        });

        if (DB::getDriverName() === 'pgsql') {
            DB::statement(
                'ALTER TABLE store_order_status_histories ADD CONSTRAINT store_order_status_histories_status_check '
                ."CHECK (status IN ('pending_payment', 'confirmed', 'processing', 'completed', 'cancelled'))",
            );
            DB::statement(
                'ALTER TABLE store_order_status_histories ADD CONSTRAINT store_order_status_histories_actor_type_check '
